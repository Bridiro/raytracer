...
impl Scene {
    ...
    pub fn set_uniforms(&self, gl: &WebGlRenderingContext, program: &WebGlProgram) -> Result<(), JsValue> {
        // Set sphere data
        ...
        for (i, sphere) in self.spheres.iter().take(10).enumerate() {
            let center_location = gl.get_uniform_location(program, &format!("u_spheres[{}].center", i));
            gl.uniform3f(
            ...
            let radius_location = gl.get_uniform_location(program, &format!("u_spheres[{}].radius", i));
            gl.uniform1f(radius_location.as_ref(), sphere.radius);
            
            let albedo_location = gl.get_uniform_location(program, &format!("u_spheres[{}].albedo", i));
            gl.uniform3f(
            ...
            let material_type_location = gl.get_uniform_location(program, &format!("u_spheres[{}].material_type", i));
            ...
            let roughness_location = gl.get_uniform_location(program, &format!("u_spheres[{}].roughness", i));
        ...
        for (i, plane) in self.planes.iter().take(5).enumerate() {
            let point_location = gl.get_uniform_location(program, &format!("u_planes[{}].point", i));
            gl.uniform3f(
            ...
            let normal_location = gl.get_uniform_location(program, &format!("u_planes[{}].normal", i));
            gl.uniform3f(
            ...
            let albedo_location = gl.get_uniform_location(program, &format!("u_planes[{}].albedo", i));
            gl.uniform3f(
                albedo_location.as_ref(),
            ...
            let material_type_location = gl.get_uniform_location(program, &format!("u_planes[{}].material_type", i));
        ...
        for (i, light) in self.lights.iter().take(4).enumerate() {
            let position_location = gl.get_uniform_location(program, &format!("u_lights[{}].position", i));
            gl.uniform3f(
            ...
            let color_location = gl.get_uniform_location(program, &format!("u_lights[{}].color", i));
            gl.uniform3f(
            ...
            let intensity_location = gl.get_uniform_location(program, &format!("u_lights[{}].intensity", i));
                        ...
                        "LIGHT" => {
                            if let Some(location) = obj.get("location").and_then(|l| l.as_array()) {
                                if location.len() >= 3 {
                                    ...
                                    let intensity = obj.get("energy").and_then(|e| e.as_f64()).unwrap_or(10.0) as f32;
